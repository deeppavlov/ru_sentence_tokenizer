...
if __name__ == '__main__':
    assert sent_tokenize('купил за 5 руб. и остался доволен.') == ['купил за 5 руб. и остался доволен.']
    assert sent_tokenize('Я ему сказал и т.к. он не послушался за 500р.') == ['Я ему сказал и т.к. он не послушался за 500р.']
    assert sent_tokenize('Ура. Ура. 500р.') == ['Ура.', 'Ура.', '500р.']
    assert sent_tokenize('Среди других её представителей — Л. Р. Зиндер, Л. В. Бондарко, М. И. Матусевич.') == \
           ['Среди других её представителей — Л. Р. Зиндер, Л. В. Бондарко, М. И. Матусевич.']
    assert sent_tokenize('И. П. Павлов.') == ['И. П. Павлов.']
    assert sent_tokenize('Павлов И. П., Сеченов И. М.') == ['Павлов И. П., Сеченов И. М.']
    assert sent_tokenize('Основателем школы является Л. В. Щерба.') == ['Основателем школы является Л. В. Щерба.']
    assert sent_tokenize('Я ему сказале: "Чтобы ничего не трогале." Но он не послушался.') == \
           ['Я ему сказале: "Чтобы ничего не трогале."', 'Но он не послушался.']
    assert sent_tokenize('Нефть за $27/барр. не снится.') == ['Нефть за $27/барр. не снится.']
    assert sent_tokenize('Сказала я. Он оглянулся.') == ['Сказала я.', 'Он оглянулся.']
    ...
    assert sent_tokenize('Где они были эти …адцать лет?') == ['Где они были эти …адцать лет?']
    assert sent_tokenize('Православие... более всего подходит на роль такой идеи...') == ['Православие... более всего подходит на роль такой идеи...']
    assert sent_tokenize('Yolka стоит 2400р. без трех копеек сто долларов, между прочим.') == ['Yolka стоит 2400р. без трех копеек сто долларов, между прочим.']
    assert sent_tokenize(
    ...
    assert sent_tokenize('Это стоило 50 т. к. вчера') == ['Это стоило 50 т. к. вчера']
    assert sent_tokenize(
